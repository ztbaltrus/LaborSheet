...
class AsyncRecycler {
    ...
    moveFolder(folderPath) {
        ...
        Utilities_1.Utilities.retryUntilTimeout(() => fsx.renameSync(folderPath, newFolderPath), maxWaitTimeMs, (e) => new Error(`Error: ${e}${os.EOL}Often this is caused by a file lock ` +
    ...
    moveAllItemsInFolder(folderPath, membersToExclude) {
        ...
        for (const memberName of fsx.readdirSync(resolvedFolderPath)) {
            const memberPath = path.join(resolvedFolderPath, memberName);
            const normalizedMemberName = memberName.toUpperCase();
            if (!excludeSet.has(normalizedMemberName)) {
                let shouldMove = false;
                try {
                    const stats = fsx.lstatSync(memberPath);
