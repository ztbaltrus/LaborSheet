...
class NpmLinkManager extends BaseLinkManager_1.BaseLinkManager {
    _linkProjects() {
        return this._readPackageTree(this._rushConfiguration.commonTempFolder).then((npmPackage) => {
    ...
    _readPackageTree(rootFolderPath) {
        return new Promise((resolve, reject) => {
            readPackageTree(rootFolderPath, (error, rootNode) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    _linkProject(project, commonRootPackage, commonPackageLookup, rushLinkJson) {
        ...
        const localProjectPackage = NpmPackage_1.NpmPackage.createLinkedNpmPackage(project.packageJson.name, commonProjectPackage.version, commonProjectPackage.dependencies, project.projectFolder);
        const queue = [];
        queue.push({
        ...
        if (localProjectPackage.children.length > 0) {
            ...
            if (fsx.existsSync(commonBinFolder)) {
                NpmLinkManager._createSymlink(commonBinFolder, projectBinFolder, BaseLinkManager_1.SymlinkKind.Directory);
            }
        }
    }
}
