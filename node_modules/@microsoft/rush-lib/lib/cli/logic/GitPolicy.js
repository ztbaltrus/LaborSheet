...
class GitPolicy {
    static check(rushConfiguration) {
        ...
        let userEmail;
        try {
            userEmail = Utilities_1.Utilities.executeCommandAndCaptureOutput('git', ['config', 'user.email'], '.').trim();
        }
        catch (e) {
...
`));
        ...
        try {
            const userName = Utilities_1.Utilities.executeCommandAndCaptureOutput('git', ['config', 'user.name'], '.').trim();
            if (userName) {
