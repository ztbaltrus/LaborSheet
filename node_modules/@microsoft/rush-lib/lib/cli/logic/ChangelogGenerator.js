    ...
    static regenerateChangelogs(allProjects, rushConfiguration) {
        allProjects.forEach(project => {
            ...
            if (fs.existsSync(markdownPath)) {
                ...
                fs.writeFileSync(path.join(project.projectFolder, CHANGELOG_MD), ChangelogGenerator._translateToMarkdown(changelog, rushConfiguration, isLockstepped), { encoding: 'utf8' });
            }
        });
    }
    /**
    ...
    static updateIndividualChangelog(change, projectFolder, shouldCommit, rushConfiguration, isLockstepped = false, isMain = true) {
        ...
        if (!changelog.entries.some(entry => entry.version === change.newVersion)) {
            ...
            if (shouldCommit) {
                ...
                fs.writeFileSync(path.join(projectFolder, CHANGELOG_MD), ChangelogGenerator._translateToMarkdown(changelog, rushConfiguration, isLockstepped), { encoding: 'utf8' });
            }
            return changelog;
        }
