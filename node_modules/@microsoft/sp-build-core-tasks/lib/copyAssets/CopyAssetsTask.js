...
const setWebpackPublicPath = require("@microsoft/set-webpack-public-path-plugin");
...
class CopyAssetsTask extends OdspGulpTask_1.default {
                ...
                if (notIgnoredExt && notEntryChunk) {
                    ...
                    if (!this.taskConfig.excludeHashFromFileNames) {
                        ...
                        if (webpackAsset.chunks.length > 0) {
                            ...
                            checksumFound = webpackAsset.chunks.reduce((state, chunkId) => state || webpackAsset.name.indexOf(webpackChunks[chunkId].hash) > -1, false);
                        }
                        else {
     ...
     * already.
     */
    _insertGlobalRegisterCodeIfNecessary(self, debug) {
        return function (file, enc, callback) {
            if (!file.isNull() && file.contents) {
                ...
                if (contents.match(new RegExp(setWebpackPublicPath.registryVariableName, 'g'))) {
                    const registerCode = setWebpackPublicPath.getGlobalRegisterCode(debug);
