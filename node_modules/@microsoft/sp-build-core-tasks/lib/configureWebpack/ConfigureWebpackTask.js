...
class ConfigureWebpackTask extends OdspGulpTask_1.default {
    ...
    getKarmaConfig(extraRules, externals, singleLocale, karmaTask) {
        const locAliases = {};
        try {
            const locResourceKeys = Object.keys(this.properties.discoveredLocalizedResources || {});
            const resolveResult = this.resolveResources(locResourceKeys, singleLocale || constants_1.default.defaultLocale, karmaTask);
            const locale = resolveResult.locales[0];
        ...
        const testAddons = {
            ...
            plugins: [new webpack.DefinePlugin({
                    ...
                    NPM_BUILD: false,
                    BUILD_NUMBER: '\'unit-test_0000-00-00.000\'',
                    DATACENTER: !this.buildConfig.args['on-prem']
                })],
            resolve: {
                alias: Object.assign({ 
                    // We need this because load-themed-styles isn’t a dependency of every project that uses it,
                    // so we need to make sure that it’s resolved correctly in tests because now we remap things
                    // to refer to it directly.
                    '@microsoft/load-themed-styles': require.resolve('@microsoft/load-themed-styles') }, locAliases)
            }
        };
        testAddons.module.rules.push(...extraRules);
