...
const _nonStandardExternalMap = {
    'react': {
        ...
        getFailoverPath: (packageRoot) => path.join(packageRoot, 'dist', 'react.js'),
    ...
    'react-dom': {
        ...
        getFailoverPath: (packageRoot) => path.join(packageRoot, 'dist', 'react-dom.js'),
...
function setLinkedExternals(gulpTask) {
    if (!gulpTask.properties.linkedExternals) {
        const linkedExternals = _getLinkedExternals(gulpTask);
...
function _getLinkedExternals(gulpTask) {
    ...
    const cumulativeManifestProcessor = cumulativeManifestProcessor_1.default.initializeFromGulpTask(gulpTask);
    const discoveredManifests = cumulativeManifestProcessor.discoverManifests(process.cwd(), cumulativeManifestProcessor_1.ForceSearch.deepSparseIgnoreFirstProject);
    const result = {};
    // Add these projects to externalized packages
    for (const manifestId in discoveredManifests) {
        if (discoveredManifests.hasOwnProperty(manifestId)) {
            for (const version in discoveredManifests[manifestId]) {
                // tslint:disable-next-line:forin
                const referencedProject = discoveredManifests[manifestId][version];
                if (discoveredManifests[manifestId].hasOwnProperty(version) && referencedProject.packageName) {
    ...
    const nonStandardExternals = _getNonStandardExternals(gulpTask.buildConfig.rootPath);
    for (const externalName in nonStandardExternals) {
        if (nonStandardExternals.hasOwnProperty(externalName)) {
...
function getVersionFunction(packageName) {
    return (projectRootPath) => {
        try {
            const resolvedPath = Resolve_1.resolvePackage(packageName, projectRootPath);
            if (resolvedPath) {
...
function _tryFillFailoverPath(projectRootPath, dependency) {
    if (!dependency.failoverPath && dependency.getFailoverPath) {
        try {
            const resolvedPath = Resolve_1.resolvePackage(dependency.name, projectRootPath);
