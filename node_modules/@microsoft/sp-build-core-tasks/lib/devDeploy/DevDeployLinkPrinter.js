...
const DEPLOY_QUERY_PARAMETERS_ENVIRONMENT_VARIALBE = 'DEV_DEPLOY_LINK_QUERY_PARAMETERS';
/**
...
function printDevDeployLink(log, metadata) {
    ...
    if (metadata.debugManifestUrl) {
        queryParameters.set(SpfxServeTask_1.DEBUG_MANIFESTS_QUERY_PARAMETER, metadata.debugManifestUrl);
    }
    const deployUrl = process.env[TEST_URL_ENVIRONMENT_VARIABLE];
    _writeDeployQueryParameters(log, queryParameters);
    if (!deployUrl) {
...
function _writeDeployQueryParameters(log, queryParameters) {
    const deployQueryParameters = _concatenateQueryParams(queryParameters);
    // This is the logging command which can write environment variables to be consumed by other tasks.
    // See https://github.com/Microsoft/vsts-tasks/blob/master/docs/authoring/commands.md
    log(`##vso[task.setvariable variable=${DEPLOY_QUERY_PARAMETERS_ENVIRONMENT_VARIALBE};]${deployQueryParameters}`);
}
