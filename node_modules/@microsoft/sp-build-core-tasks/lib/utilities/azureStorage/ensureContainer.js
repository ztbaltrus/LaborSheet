"use strict";
/**
 * @file ensureBlobService.d.ts
 * @Copyright (c) Microsoft Corporation.  All rights reserved.
 *
 * Creates a container on an Azure Blob Service connection object
 */
Object.defineProperty(exports, "__esModule", { value: true });
function ensureContainer(blobService, containerName, containerOptions, log = console.log) {
    if (!containerOptions) {
        containerOptions = {
            publicAccessLevel: 'blob'
        };
    }
    return new Promise((resolve, reject) => {
        try {
            blobService.createContainerIfNotExists(containerName, containerOptions, (error, result, response) => {
                if (error) {
                    reject(new Error(`Unable to create container: ${containerName}`));
                }
                if (result) {
                    log(`Created container: ${containerName}`);
                }
                else {
                    log(`Container {${containerName}} exists!`);
                }
                resolve(blobService);
            });
        }
        catch (error) {
            reject(new Error(error));
        }
    });
}
exports.default = ensureContainer;
//# sourceMappingURL=ensureContainer.js.map