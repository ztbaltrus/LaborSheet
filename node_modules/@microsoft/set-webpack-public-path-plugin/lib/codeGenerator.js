...
function getSetPublicPathCode(options, emitWarning) {
    ...
    if (options.regexName) {
        ...
        const escapedRegex = options.regexName.replace(/\\/, '\\\\');
        lines = [
            `var scripts = document.getElementsByTagName('script');`
        ];
        const regexInitializationSnippet = `new RegExp('${escapeSingleQuotes(escapedRegex)}', 'i')`;
...
function getGlobalRegisterCode(debug = false) {
    ...
    const joinedScript = joinLines(lines);
    if (debug) {
        return `${os_1.EOL}${joinedScript}`;
    }
    else {
        const minifyOutput = uglify.minify(joinedScript, {
            compress: {
                dead_code: true
            }
        });
        return `${os_1.EOL}${minifyOutput.code}`;
    }
}
