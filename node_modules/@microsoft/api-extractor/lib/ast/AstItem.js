...
class AstItem {
    constructor(options) {
        ...
        const sourceFileText = this.declaration.getSourceFile().text;
        let aedocCommentRange;
        if (options.aedocCommentRange) {
            ...
            const ranges = TypeScriptHelpers_1.TypeScriptHelpers.getJSDocCommentRanges(this.declaration, sourceFileText) || [];
            if (ranges.length > 0) {
        ...
        if (aedocCommentRange) {
            ...
            const rawComment = sourceFileText.substring(aedocCommentRange.pos, aedocCommentRange.end);
            if (rawComment) {
                aedoc = TypeScriptHelpers_1.TypeScriptHelpers.extractJSDocContent(rawComment, this.reportError);
            }
        }
        this.documentation = new ApiDocumentation_1.ApiDocumentation(aedoc, this.context.docItemLoader, this.context, this.reportError, this.warnings);
    }
    /**
    ...
    onCompleteInitialization() {
        ...
        if (this.documentation.releaseTag === ReleaseTag_1.ReleaseTag.None) {
            ...
            if (this.parentContainer && this.parentContainer.kind === AstItemKind.Package) {
                ...
                this.documentation.releaseTag = ReleaseTag_1.ReleaseTag.Public;
            }
        }
    }
    /**
    ...
    _analyzeTypeReference(typeReferenceNode) {
        ...
        const currentSymbol = TypeScriptHelpers_1.TypeScriptHelpers.followAliases(symbol, this.typeChecker);
        ...
        const typeReferencePackageJson = this.context.packageJsonLookup
        ...
        const parsedPackageName = node_core_library_1.PackageName.parse(typeReferencePackageName);
        ...
        const apiDefinitionRef = ApiDefinitionReference_1.ApiDefinitionReference.createFromParts(apiDefinitionRefParts);
