...
class Tokenizer {
    ...
    _tokenizeDocs(docs) {
        ...
        const docEntries = TypeScriptHelpers_1.TypeScriptHelpers.splitStringWithRegEx(docs.replace(/\r/g, ''), // CRLF -> LF
        ...
        for (let i = 0; i < docEntries.length; i++) {
            ...
            if (trimmed.charAt(0) === '@') {
                token = new Token_1.Token(Token_1.TokenType.BlockTag, trimmed);
            }
                ...
                token = new Token_1.Token(Token_1.TokenType.Text, '', untrimmed);
            }
            if (token) {
                tokens.push(token);
            }
    ...
    _tokenizeInline(docEntry) {
        ...
        if (tokenChunks[0] === '@link') {
            ...
            const token = new Token_1.Token(Token_1.TokenType.InlineTag, '@link', tokenChunks.join(' '));
            return token;
        }
        else if (tokenChunks[0] === '@inheritdoc') {
            tokenChunks.shift(); // Gets rid of '@inheritdoc'
            const token = new Token_1.Token(Token_1.TokenType.InlineTag, '@inheritdoc', tokenChunks.join(' '));
            return token;
        }
